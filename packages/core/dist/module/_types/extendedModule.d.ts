 ...
 */
export declare type ExtractModuleSettings<M extends ParameterizedModule> = M["settings"] extends DeepReadonly<infer S> ? S : never;
/**
 * Filters out any methods from a module that should be overwritten
 */
export declare type FilterModule<M extends ParameterizedModule> = Omit<M, "changeState" | "changeSettings">;
/**
...
export declare type ExtendedModule<MC extends ParameterizedModuleConfig, M extends ParameterizedModule> = {
    changeState(state: DataChange<OrEmpty<MC["state"]> & ExtractModuleState<M>>): Promise<void>;
    changeSettings(settings: DataChange<OrEmpty<SettingsConfigSetData<MC["settings"]> & ExtractModuleSettings<M>>>, conditions?: SettingsConditions): Promise<void>;
...
export declare type ExtendedModuleClass<MC extends ParameterizedModuleConfig, X extends Constructor<any>> = {
    new (request: ParameterizedModuleRequestData, moduleID: ModuleID, state: ModuleState, parents: ModuleProxy[]): ExtendedModule<MC, GetConstructed<X>>;
} & typeof Module;
