 ...
 */
export class RemoteModuleProxy {
    // Keep track of the moduleID that this is a proxy for
    ...
    public static createClass(
        moduleCls: ExtendsClass<typeof Module>
    ): ExtendsClass<typeof RemoteModuleProxy> {
        // Create the proxy class
        const cls = this.createNamedClass((moduleCls as any).name, RemoteModuleProxy);
        ...
        ExtendedObject.forEach(methods, (name, method) => {
            cls.prototype[name] = function(this: RemoteModuleProxy, ...args: any[]) {
                IpcRenderer.send("WindowManager.moduleCall", this.moduleID, name, args);
            };
        });
    ...
    public static createInstance<
        ...
        P extends RemoteModuleProxy = RemoteModuleProxy
    >(moduleID: ModuleID | string): P & M {
        return new this(moduleID) as any;
    }
}
